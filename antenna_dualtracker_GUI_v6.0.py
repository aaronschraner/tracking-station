...
db = MySQLdb.connect(host="153.90.202.51",user="scott",passwd="Jewe1947",db="freemanproject")
IMEI = "300234064807740"
...
##        return QtGui.QWidget.eventFilter(self,widget,event)
    ...
    def pointToTarget(self):
        ...
        ## MARK ##
        ## shifting the basic 'altitude value' up by 
        ## groundAlt += 200;        
...
def getGPS():
    global receivedTime, receivedLat, receivedLon, receivedAlt, bearingLog, elevationLog, losLog
    pi_host = "10.0.0.2" #Change this to the IP of the pi
    pi_port = "2947"
    session = gps.gps(pi_host, pi_port)
    ...
    while True:
            try:
                ...
                if report['class'] == 'TPV':
                    #populate the shit
                    remoteLat = session.fix.latitude
                    remoteLon = session.fix.longitude
                    remoteAlt = session.fix.altitude
